...
console.log(
  ' TIKTOK ANDROID RUNTIME - REAL DB READ-ONLY SMOKE',
);
...
const provider:
  TikTokAutomationProvider = {
    ...
    async checkRelationship() {
      throw new Error(
        'Relationship handler must not execute during SELECT-only runtime smoke.',
      );
    },

    async follow() {
      throw new Error(
        'FOLLOW must not execute during SELECT-only runtime smoke.',
      );
    },

    async unfollow() {
      throw new Error(
        'UNFOLLOW must not execute during SELECT-only runtime smoke.',
      );
    },
  };
  ...
  new TikTokRuntime({
    ...
    schedulerRunner:
      async options => {
        ...
        console.log(
          `REAL_DB_DUE_ANDROID_ACTIONS=${actions.length}`,
        );
...
try {
  ...
  console.log(
    'TIKTOK_ANDROID_RUNTIME_DB_SMOKE=PASS',
  );

  console.log(
    'PROVIDER_SCOPE_ANDROID_ONLY=PASS',
  );
