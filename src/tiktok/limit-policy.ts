...
export class TikTokLimitPolicy {
  ...
  async evaluate(
    ...
    /*
     * minInterval and cooldown are both post-success spacing
     * constraints.
     *
     * When both are active, the later retry time must win.
     * Returning the first matching rule could otherwise wake the
     * scheduler too early only to be blocked by the second rule.
     */
    if (lastSuccessAt) {
      ...
      if (
        ...
        if (
          cooldownRetryAt.getTime() >
            now.getTime() &&
          (
            spacingRetryAt === null ||
            cooldownRetryAt.getTime() >
              spacingRetryAt.getTime()
          )
        ) {
