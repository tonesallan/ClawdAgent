  ...
  for (const action of actions) {
    ...
    if (
      ...
      if (!policyDecision.allowed) {
        ...
        /*
         * A blocked action must always leave the current
         * due queue.
         *
         * Policies such as allowedHours already supply retryAt.
         *
         * Policies such as enabled=false intentionally do not
         * have a business retry time, so they receive a short
         * persistent recheck window instead.
         *
         * This prevents:
         *
         * - scheduler hot loops;
         * - starvation of later actions;
         * - needless DB churn;
         * - accidental attempt consumption.
         */
        const retryAt =
      ...
      try {
        ...
        if (exhausted) {
           ...
           * Terminal failure.
           *
           * No further scheduling occurs after maxAttempts.
           */
          await transitionTikTokAction(
            action.id,
            {
          ...
          /*
           * Technical failure with persistent exponential
           * backoff.
           *
           * Production uses the actual failure time.
           * Tests may inject `now` for deterministic behavior.
           */
