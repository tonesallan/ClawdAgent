...
export function createFollowBackSchedulerHandler(
  ...
  return async (action) => {
     ...
     * Safety rule:
     *
     * UNKNOWN cannot be interpreted as
     * "did not follow back".
     *
     * NOT_FOLLOWING is also ambiguous for the 48h rule,
     * because our system may no longer be following the
     * target for another reason.
     *
     * In both cases we fail the check so the scheduler
     * retry mechanism can handle it rather than creating
     * an incorrect UNFOLLOW candidate.
     */
    if (
      relationshipState ===
